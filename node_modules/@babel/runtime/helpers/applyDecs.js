...
function applyMemberDec(ret, base, decInfo, name, kind, isStatic, isPrivate, metadataMap, initializers) {
  ...
  if ("function" == typeof decs) void 0 !== (newValue = decs(value, ctx)) && (assertValidReturnValue(kind, newValue), 0 === kind ? initializer = newValue : 1 === kind ? (initializer = newValue.initializer, get = newValue.get || value.get, set = newValue.set || value.set, value = {
    get: get,
    set: set
  }) : value = newValue);else for (var i = 0; i < decs.length; i++) {
    var newInit;
...
function applyMemberDecs(ret, Class, protoMetadataMap, staticMetadataMap, decInfos) {
  for (var protoInitializers, staticInitializers, existingProtoNonFields = new Map(), existingStaticNonFields = new Map(), i = 0; i < decInfos.length; i++) {
    ...
    if (Array.isArray(decInfo)) {
      ...
      if (isStatic ? (base = Class, metadataMap = staticMetadataMap, kind -= 5, staticInitializers || (staticInitializers = []), initializers = staticInitializers) : (base = Class.prototype, metadataMap = protoMetadataMap, protoInitializers || (protoInitializers = []), initializers = protoInitializers), 0 !== kind && !isPrivate) {
  ...
  protoInitializers && pushInitializers(ret, protoInitializers), staticInitializers && pushInitializers(ret, staticInitializers);
}
...
function applyClassDecs(ret, targetClass, metadataMap, classDecs) {
  ...
  }, createMetadataMethodsForProperty(metadataMap, 0, name)), i = 0; i < classDecs.length; i++) {
